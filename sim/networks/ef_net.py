...
class simple_EF_block(torch.nn.Module):
    def __init__(self, in_channels, kernel_size, cb):
        ...
        stacked_kernel_as_m = torch.cat((id_kernel_as_m, conv2_kernel_as_m, bias2_as_m), axis=1)
    ...
    def forward(self, x):
        ...
        for i in range(self.N):
            ...
            if self.cbon:
                x2 = self.conv2.apply(x_temp, x, self.conv2_kernel, self.bias2, self.kernel_and_id)
            else:
                x2 = x + self.h * torch.nn.functional.conv2d(x_temp, self.conv2_kernel, self.bias2, padding=1)
    ...
    def remap(self):
        ...
        self.conv1.remap()
        self.kernel_and_id.remap(torch.cat((id_kernel_as_m, conv2_kernel_as_m, bias2_as_m), axis=1))
