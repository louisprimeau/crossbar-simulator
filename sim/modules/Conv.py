...
class Conv2d(torch.nn.Module):
    ...
    def forward(self, inp):
        ...
        if self.cbon:
            out = conv2d.apply(inp, self.conv_kernel, self.bias, self.kernel, self.padding, self.stride)
        else:
            out = f.conv2d(inp, self.conv_kernel, self.bias, stride=self.stride, padding=self.padding)
        return out
